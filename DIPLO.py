...
# Bearer Token; Bluesky public search needs no key; Mastodon accepts an optional
# user access token. Then choose an LLM provider for translation and enrichment.
...
def prompt_data_sources() -> List[str]:
    ...
    print("\nSocial-data sources:")
    print("  1. X API")
    print("  2. Bluesky public API (no key required)")
...
def fetch_bluesky_page(
    ...
    """Fetch and normalize one public Bluesky search page."""
    endpoint = (
...
def run_bluesky_collection(
    ...
    print("Bluesky public API collection started.")
    session = create_requests_session()
        ...
        for page_num in range(1, max_pages_per_query + 1):
            ...
            for post in posts:
                ...
                record = make_record_from_post(
                    post=post,
                    source_mode="api",
                    source_host="public.api.bsky.app",
                    client=client,
                    openai_model=openai_model,
                    target_language=target_language,
                    translate=translate,
